        ...
        while self.ser.in_waiting > 0:
            ...
            while True:
                if len(self._read_buffer) < 3:
                    break
                if self._read_buffer[0] != self.startByte:                        # Wait for start byte
                    self._read_buffer.pop(0)
                    continue
                expected_length = self._read_buffer[1]                            # Log the size of the incoming packet from length byte
                if len(self._read_buffer) < expected_length + 3:                  # Check to make sure incoming byte meets expected size
                    break
